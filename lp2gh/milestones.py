...
def import_(repo, milestones, milestones_map=None):
    ...
    milestones_map = milestones_map or {}

    o = {}
    e = exporter.Exporter()
    ms = repo.milestones()
    closed_ms = repo.milestones(state='closed')
    if (len(ms.datalist) > 0):
        open_ms_by_id = dict((x['number'], x) for x in ms.datalist)
        closed_ms_by_id = dict((x['number'], x) for x in closed_ms.datalist)
    ...
    for x in milestones:
        ...
        try:
            ...
            elif ms is not None and ms.datalist is not None and (rv := next((m for m in ms.datalist if x['name'] == m['title']), None)) is not None:
                pass
            else:
                rv = ms.append(**params)
            o[x['name']] = rv['number']
